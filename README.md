...
- [judge_rails](https://github.com/just-the-v/judge_rails) - Ruby gem where Jev answers Noul, Choice, and Score questions about a record's text and each answer is stored in an ordinary ActiveRecord column with a jsonb provenance sidecar; code owns the thresholds, the SQL scopes such as `urgency_above(0.8)`, and a background recompute when the source text changes.
