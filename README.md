...
- [jev-style](https://github.com/lawrence3699/jev-style) - Open 0.8B Qwen3.5 decision model installed with `pip install "jev-style[torch]"` (or `[mlx]` on Apple silicon) that answers Choice, Score and Noul questions locally behind a `/v1/systemone`-compatible server, while code owns the thresholds (its Claude Code guard maps four Noul checks and a risk Score to allow, ask or deny, and falls back to ask when the model call fails); its README reports hosted Jev as more accurate on every published comparison set.
